...
def get_queues_bound_to_fanout_exchange(exchange_name):
    ...
    # 构建 API 请求的 URL
    url = f"{api_url}/exchanges/producer/{exchange_name}/bindings/source"
...
def sche_time_run(resolve_type):
    ...
    domains, dns_servers = get_domain_dns()
    # 使用schedule库设置定时任务
    for time_str in scheduled_times:
        schedule.every().day.at(time_str).do(main, domains, dns_servers, resolve_type)
...
def update_schedule(new_times, resolve_type = 'A'):
...
class dns_producer:
    def __init__(self, resolve_type):
        ...
        self.channel.exchange_declare(exchange='test', exchange_type='fanout', durable=True, passive=True)
        self.bind_consumer = get_queues_bound_to_fanout_exchange('test')
...
def get_domain_dns():
    ...
    conn.select_db("dns")
    # 执行查询操作
    cursor.execute('SELECT dns_server_ip FROM important_dns_server')
    # 获取查询结果，返回元组
    dns = cursor.fetchall()
    cursor.execute('SELECT domain_name FROM important_domain')
...
if __name__ == '__main__':
    ...
    resolve_type = sys.argv[3]
    # 支持 A, AAAA, ANY, CAA, CNAME, MX, NAPTR, NS, PTR, SOA, SRV, TXT
    # 测试过A、AAAA、MX、CNAME、NS
    main(domains, dns_servers, resolve_type)
    # 定时
    # new_times = ['10:36']
    # update_schedule(new_times)
