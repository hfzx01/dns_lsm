...
async def resolve_domain(domain, dns_server, record_type):
    ...
    try:
        ...
        if isinstance(result, list):  # 对于 'A'、'AAAA' 等类型
            return {dns_server: [str(r.host) for r in result]}
        elif hasattr(result, 'cname'):  # 对于 'CNAME' 查询返回单一对象的情况
            return {dns_server: [str(result.cname)]}
        else:
            return {dns_server: ["Unexpected result format"]}
    except aiodns.error.DNSError as e:
        return {dns_server: [f"Error: {str(e)}"]}
...
def allocating_task(task_id, domains, dns_servers, resolve_type, process_times=0.5, coroutine_num=100, batch_size=20000):
    """
    多进程+协程分配任务，按批次处理DNS服务器。
    每批处理batch_size数量的DNS服务器，处理完一批后再处理下一批。
    域名列表在所有批次中保持不变。
    """
    ...
    # 计算总批次数量，按 DNS 服务器分批
    num_batches = math.ceil(len(dns_servers) / batch_size)
    all_results = {}
    ...
    for batch_num in range(num_batches):
        ...
        # 获取当前批次的 DNS 服务器列表
        batch_dns_servers = dns_servers[batch_num * batch_size: (batch_num + 1) * batch_size]

        # 将当前批次的 DNS 服务器列表分配给多个进程，处理固定的域名列表
        avg_dns_list = list_split(batch_dns_servers, math.ceil(len(batch_dns_servers) / process_num))
        try:
            p_list = [
                QueryProcess(task_id, f'Process_{i}', q, domains, each_dns_list, resolve_type, coroutine_num)
                for i, each_dns_list in enumerate(avg_dns_list)
            ]
            for p in p_list:
                p.start()
            for p in p_list:
...
class Consumer:
    ...
    def receive_message(self):
        ...
        channel.exchange_declare(exchange='service', durable=True, exchange_type='fanout')
        result = channel.queue_declare(queue=self.queue_name, exclusive=True)
        channel.queue_bind(exchange='service', queue=result.method.queue)
