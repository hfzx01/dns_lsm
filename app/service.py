...
from range_analysis import get_range
...
def get_queues_bound_to_fanout_exchange(exchange_name):
    ...
    # 构建 API 请求的 URL
    url = f"{api_url}/exchanges/service/{exchange_name}/bindings/source"
...
def sche_time_run(resolve_type):
    # 读取配置文件中的定时执行时间列表
    ...
    for time_str in scheduled_times:
        schedule.every().day.at(time_str).do(main, resolve_type)
...
class dns_producer:
    def __init__(self, resolve_type):
        ...
        self.connection = pika.BlockingConnection(
            pika.ConnectionParameters(host='8.210.155.15', port=5672, virtual_host='service', heartbeat=0,
                                      credentials=self.credentials))
        self.channel = self.connection.channel()
        self.channel.exchange_declare(exchange='service', exchange_type='fanout', durable=True, passive=True)
        self.bind_consumer = get_queues_bound_to_fanout_exchange('service')
    ...
    def on_response(self, ch, method, properties, body):
        if self.corr_id == properties.correlation_id:
            ...
            # 创建文件夹并保存结果到 JSON 文件
            create_folder_and_save_result(self.task_id, head, body, self.resolve_type)
            self.fp = f"service_task/task_{self.task_id}"
...
def create_folder_and_save_result(task_id, head, body, resolve_type):
    # 创建文件夹
    folder_name = f"service_task/task_{task_id}"
    os.makedirs(folder_name, exist_ok=True)

    # JSON 文件名为节点名
    file_name = f'{head}_{resolve_type}.json'
...
def get_dns():
    ...
    with open(file_path, 'r') as file:
        for line in file.readlines():
            ...
            dns_list.append(content['target'])
    return dns_list


def main(domains, dns_servers, resolve_type):
    producer = dns_producer(resolve_type)
    domains.sort()
    dns_servers.sort()
    print(len(dns_servers))
    message = {'domains': domains, "dns_servers": dns_servers, "resolve_type": resolve_type}
...
if __name__ == '__main__':
    ...
    # dns_servers = ["211.136.192.6", "218.202.3.3", "221.179.38.7", '114.114.114.114', '8.8.8.8', "1.2.4.8"]
    # dns_servers = ['114.114.114.114', '8.8.8.8',"1.2.4.8"]
    ...
    # domains = list(df['domain'])[:750000]
    # 读excel
    # data = pd.read_excel('../dns_entity_info_new.xlsx')  # 替换为你的 Excel 文件路径
    # dns_servers = data.iloc[:, 0].tolist()
    # 演示使用
    domains = read_txt(sys.argv[1])
    dns_servers = read_txt(sys.argv[2])
    # 支持 A, AAAA, ANY, CAA, CNAME, MX, NAPTR, NS, PTR, SOA, SRV, TXT
    # 测试过A、AAAA、MX、CNAME、NS
    fp = main(domains, dns_servers, 'A')
    get_range(fp)
    # new_times = ['17:42']
    # update_schedule(new_times,'NS')
